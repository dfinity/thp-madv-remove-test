...
fn run_iteration() -> usize {
    ...
    // Create memfd.
    let fd = unsafe { libc::memfd_create(c"thp_test".as_ptr(), 0) };
    ...
    let readers: Vec<_> = (0..NUM_READER_THREADS)
        .map(|tid| {
            ...
            std::thread::spawn(move || {
                ...
                let mut _local_failures = 0usize;
                let mut _pass = 0u64;
...
fn check_page(base: *mut u8, page_idx: usize) -> bool {
    ...
    if first_8 != expected_idx_bytes {
        let _got_idx = u64::from_le_bytes(first_8);
    ...
    // // Spot-check the fill byte at several offsets.
    // for &offset in &[8, 64, PAGE_SIZE / 2, PAGE_SIZE - 1] {
    //     let val = unsafe { ptr::read(page_ptr.add(offset)) };
    //     if val != fb {
    //         let all_zero = unsafe { (8..PAGE_SIZE).all(|off| ptr::read(page_ptr.add(off)) == 0) };
    //         if all_zero {
    //             eprintln!(
    //                 "CORRUPTED: page {} (huge page {}): header intact but body is ALL ZEROS",
    //                 page_idx,
    //                 page_idx * PAGE_SIZE / HUGE_PAGE_SIZE,
    //             );
    //         } else {
    //             eprintln!(
    //                 "CORRUPTED: page {} (huge page {}) at offset {}: expected {:#04x}, got {:#04x}",
    //                 page_idx,
    //                 page_idx * PAGE_SIZE / HUGE_PAGE_SIZE,
    //                 offset,
    //                 fb,
...
fn verify_pages(base: *mut u8, is_punched: &[bool]) -> usize {
    let mut failures = 0;
    for (page_idx, &punched) in is_punched.iter().enumerate().take(TOTAL_PAGES) {
        if punched {
            continue;
        }
        if !check_page(base, page_idx) {
